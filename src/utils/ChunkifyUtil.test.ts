...
import { ChunkifyUtil } from './ChunkifyUtil';
...
describe('ChunkifyUtil', () => {
  test('chunkify divides entries into correct number of chunks', () => {
    const chunkSize = 1000;
    const chunks = ChunkifyUtil.chunkifyArray(testEntries, chunkSize);
    expect(chunks.length).toBeGreaterThan(0);
    for (const chunk of chunks) {
      const chunkString = JSON.stringify(chunk);
      expect(chunkString.length).toBeLessThanOrEqual(chunkSize);
    }
  });
});